...
export async function detectConflicts(
  ...
  // Program active + rate present.
  const prog = await pool.query<{ is_active: boolean; name: string }>(
    `SELECT is_active, name FROM programs WHERE id = $1`,
    [draft.programId],
  );
  if (!prog.rows[0]) w.push({ code: "program_missing", severity: "error", message: "The program does not exist." });
  else if (!prog.rows[0].is_active) w.push({ code: "program_inactive", severity: "warning", message: `Program ${prog.rows[0].name} is inactive.` });
  ...
  if (isGroup) {
    const capable = await pool.query<{ is_group_capable: boolean }>(
      `SELECT is_group_capable FROM programs WHERE id = $1`,
      [draft.programId],
    );
    if (capable.rows[0] && !capable.rows[0].is_group_capable) {
      w.push({ code: "program_not_group", severity: "warning", message: "This program is not marked group-capable." });
    }
  }
  return w;
}
    ...
    const { rows } = await client.query<{ id: string }>(
      `INSERT INTO scheduled_sessions
        (series_id, employee_id, program_id, service_type, session_date, start_time, end_time,
         ...
         expected_internal_amount, status, override_reason, warnings, source, notes, created_by_user_id)
