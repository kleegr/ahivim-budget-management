...
export async function setGlobalPortalRoleAssignment(
  ...
  const account = await pool.query(`SELECT id FROM users WHERE id = $1 AND is_active = true`, [input.userId]);
  if (!account.rows[0]) return fail("not_found", "That active account no longer exists.");

  return inTransaction(pool, async (client) => {
    if (input.role === "owner" && !isActive) {
      await client.query("LOCK TABLE user_portal_roles IN SHARE ROW EXCLUSIVE MODE");
      const owners = await client.query<{ active_owner_count: number | string }>(
