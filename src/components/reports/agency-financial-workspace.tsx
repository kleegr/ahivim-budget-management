...
export default function AgencyFinancialWorkspace({
  ...
  return (
    <>
      ...
      {view === "checks" ? (
        <section className="card overflow-hidden">
          <header className="flex flex-wrap items-start justify-between gap-2 border-b border-[var(--color-rule)] px-5 py-3.5"><div><h2 className="display text-base font-semibold">Verified direct-pay checks</h2><p className="mt-1 text-sm text-[var(--color-ink-soft)]">Each check is counted once. Withholding is exactly verified gross minus verified net. A missing gross is disclosed and excluded from withholding; employee share is calculated from net.</p></div><Link className="btn btn-sm btn-secondary" href="/masser?view=checks"><HandCoins className="h-4 w-4" aria-hidden /> Open checks</Link></header>
