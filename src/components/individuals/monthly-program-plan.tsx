...
export default function MonthlyProgramPlan({ input, individualId, programId, canOpenTransactions }: {
  ...
  return <section className="border-t border-[var(--color-rule)] p-4 sm:p-5" aria-label="Monthly actuals and remaining plan">
    ...
    <p className="mt-3 text-xs text-[var(--color-ink-soft)]">{plan.method}. Targets distribute remaining positive authorization through the earlier of the authorization end and day before renewal. Current-month target covers only the remaining days. Pending unmatched future visits count toward the target; they are not proof of completed service.</p>
  </section>;
}
