...
import {
  ...
  type AgencyIndividualMembershipInput,
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ ok: true, data: await listAgencyIndividualMemberships(authorized.pool, id) });
  } catch (error) {
    return jsonError(redactError(error, "Could not load the agency individual roster."), 500);
  }
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const input: AgencyIndividualMembershipInput = {
      ...
      managesBudget: body.managesBudget === true || body.managesBudget === "true",
      billsServices: body.billsServices === true || body.billsServices === "true",
    ...
    const result = await setAgencyIndividualMembership(
      authorized.pool,
      id,
      input,
      authorized.user.id,
    ...
    return jsonError(redactError(error, "Could not update the agency individual roster."), 500);
  }
}
