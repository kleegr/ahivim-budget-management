...
| Portal statements, trends, and schedules | IMPLEMENTED | Individual/parent portal supports selected-month detail, a twelve-month default trend (bounded to 24), capability-gated categories, printable statement, and CSV download without employee/check/tax/gross/net leakage. Individual, parent, and employee portals show the next five permitted visits and an exact-subject full 60-day schedule. Group participants are hidden from individual/parent views; employee views receive only their assigned participants. Agency portals expose only granted detail. | Verify empty months, renewal boundaries, downloads, print layout, upcoming schedules, mobile layout, and category denials in production. |
