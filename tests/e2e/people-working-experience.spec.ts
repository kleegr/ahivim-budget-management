...
test('People working flow retains bulk selection, shows separate program overruns and monthly plans, and creates an assignment from prior work', async ({ page }, testInfo) => {
  ...
  try {
    ...
    await expect(bulk.getByText('2 selected people', { exact: true })).toBeVisible();
    ...
    await bulk.getByRole('button', { name: 'Save changes', exact: true }).click();
    ...
    await expect(page.getByRole('tab', { name: primary.name, exact: true })).toBeVisible();
    await expect(page.getByRole('tab', { name: secondary.name, exact: true })).toBeVisible();
    await expect(page.getByRole('region', { name: 'Monthly actuals and remaining plan' }).filter({ visible: true })).toHaveCount(1);
    const monthlyDrill = page.getByRole('region', { name: 'Monthly actuals and remaining plan' }).filter({ visible: true }).getByRole('link', { name: /^Credited budget hours/ }).first();
    ...
    await expect(page.getByRole('tab', { name: outside.name, exact: true })).toBeVisible();
