...
const mocks = vi.hoisted(() => ({
  ...
  recoverSourceBase: vi.fn(), refreshSettlementObligations: vi.fn(),
}));
vi.mock("@/lib/auth/session", () => ({ apiUser: mocks.apiUser }));
vi.mock("@/lib/db", () => ({ getPool: mocks.getPool }));
vi.mock("@/lib/sheets/base-recovery", () => ({ recoverSourceBase: mocks.recoverSourceBase }));
...
import { POST } from "@/app/api/sync/source-base-recovery/route";
...
describe("Source base recovery route authorization boundary", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mocks.readJson.mockResolvedValue({ action: "accept", reason: "Exact source reviewed", operationKey: "operation-1", sourceHash: "source-hash", transactionIds: [conflictId] });
    mocks.recoverSourceBase.mockResolvedValue({ ok: true, data: { transactionId: "transaction-1", acceptanceAuditId: "audit-1", auditId: "audit-1", status: "accepted", alreadyApplied: false } });
  });
  ...
  it("rejects origin before authentication, request-body parsing, or database access", async () => {
    ...
    expect(mocks.recoverSourceBase).not.toHaveBeenCalled();
  });
  ...
  for (const role of ["manager", "admin"]) {
    for (const action of ["accept", "undo"]) {
      it(`${role} ${action} records the actual actor, preserves the review request and never refreshes settlements`, async () => {
        mocks.apiUser.mockResolvedValue({ id: effectiveId, actorId, role });
        const input = { action, reason: "Exact source reviewed", operationKey: "operation-1", sourceHash: "source-hash", transactionIds: [conflictId], ...(action === "undo" ? { acceptanceAuditId: "accepted-audit" } : {}) };
        mocks.readJson.mockResolvedValue(input);
        const response = await POST(request());
        expect(response.status).toBe(200);
        expect(await response.json()).toMatchObject({ ok: true, data: { status: "accepted" } });
        expect(mocks.apiUser).toHaveBeenCalledExactlyOnceWith("manager");
        expect(mocks.sameOriginOrFail.mock.invocationCallOrder[0]).toBeLessThan(mocks.apiUser.mock.invocationCallOrder[0]);
        expect(mocks.recoverSourceBase).toHaveBeenCalledExactlyOnceWith(pool, { ...input, acceptanceAuditId: action === "undo" ? "accepted-audit" : undefined }, actorId);
        expect(mocks.refreshSettlementObligations).not.toHaveBeenCalled();
      });
    }
  }
